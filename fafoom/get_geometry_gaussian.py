...
opt_param_line_nums = get_geometry(text, '!   Optimized Parameters   !')
# Then find all lines that have "Input orientation:"
input_orient_line_nums = get_geometry(text, 'Standard orientation')
# And all lines that have the ---- which we will use to find the ends of the coordinate sections
end_coor_line_nums = get_geometry(text, '---------')
...
outfile = open('gaussian_molecule.xyz','w')
